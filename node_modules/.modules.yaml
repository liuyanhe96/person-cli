hoistPattern:
  - '*'
hoistedDependencies:
  /@inquirer/checkbox/4.2.0_@types+node@24.1.0:
    '@inquirer/checkbox': private
  /@inquirer/confirm/5.1.14_@types+node@24.1.0:
    '@inquirer/confirm': private
  /@inquirer/core/10.1.15_@types+node@24.1.0:
    '@inquirer/core': private
  /@inquirer/editor/4.2.15_@types+node@24.1.0:
    '@inquirer/editor': private
  /@inquirer/expand/4.0.17_@types+node@24.1.0:
    '@inquirer/expand': private
  /@inquirer/figures/1.0.13:
    '@inquirer/figures': private
  /@inquirer/input/4.2.1_@types+node@24.1.0:
    '@inquirer/input': private
  /@inquirer/number/3.0.17_@types+node@24.1.0:
    '@inquirer/number': private
  /@inquirer/password/4.0.17_@types+node@24.1.0:
    '@inquirer/password': private
  /@inquirer/rawlist/4.1.5_@types+node@24.1.0:
    '@inquirer/rawlist': private
  /@inquirer/search/3.0.17_@types+node@24.1.0:
    '@inquirer/search': private
  /@inquirer/select/4.3.1_@types+node@24.1.0:
    '@inquirer/select': private
  /@inquirer/type/3.0.8_@types+node@24.1.0:
    '@inquirer/type': private
  /@jridgewell/gen-mapping/0.3.12:
    '@jridgewell/gen-mapping': private
  /@jridgewell/resolve-uri/3.1.2:
    '@jridgewell/resolve-uri': private
  /@jridgewell/source-map/0.3.10:
    '@jridgewell/source-map': private
  /@jridgewell/sourcemap-codec/1.5.4:
    '@jridgewell/sourcemap-codec': private
  /@jridgewell/trace-mapping/0.3.29:
    '@jridgewell/trace-mapping': private
  /@kwsites/file-exists/1.1.1:
    '@kwsites/file-exists': private
  /@kwsites/promise-deferred/1.1.1:
    '@kwsites/promise-deferred': private
  /@rollup/pluginutils/5.2.0_rollup@4.46.1:
    '@rollup/pluginutils': private
  /@rollup/rollup-android-arm-eabi/4.46.1:
    '@rollup/rollup-android-arm-eabi': private
  /@rollup/rollup-android-arm64/4.46.1:
    '@rollup/rollup-android-arm64': private
  /@rollup/rollup-darwin-arm64/4.46.1:
    '@rollup/rollup-darwin-arm64': private
  /@rollup/rollup-darwin-x64/4.46.1:
    '@rollup/rollup-darwin-x64': private
  /@rollup/rollup-freebsd-arm64/4.46.1:
    '@rollup/rollup-freebsd-arm64': private
  /@rollup/rollup-freebsd-x64/4.46.1:
    '@rollup/rollup-freebsd-x64': private
  /@rollup/rollup-linux-arm-gnueabihf/4.46.1:
    '@rollup/rollup-linux-arm-gnueabihf': private
  /@rollup/rollup-linux-arm-musleabihf/4.46.1:
    '@rollup/rollup-linux-arm-musleabihf': private
  /@rollup/rollup-linux-arm64-gnu/4.46.1:
    '@rollup/rollup-linux-arm64-gnu': private
  /@rollup/rollup-linux-arm64-musl/4.46.1:
    '@rollup/rollup-linux-arm64-musl': private
  /@rollup/rollup-linux-loongarch64-gnu/4.46.1:
    '@rollup/rollup-linux-loongarch64-gnu': private
  /@rollup/rollup-linux-ppc64-gnu/4.46.1:
    '@rollup/rollup-linux-ppc64-gnu': private
  /@rollup/rollup-linux-riscv64-gnu/4.46.1:
    '@rollup/rollup-linux-riscv64-gnu': private
  /@rollup/rollup-linux-riscv64-musl/4.46.1:
    '@rollup/rollup-linux-riscv64-musl': private
  /@rollup/rollup-linux-s390x-gnu/4.46.1:
    '@rollup/rollup-linux-s390x-gnu': private
  /@rollup/rollup-linux-x64-gnu/4.46.1:
    '@rollup/rollup-linux-x64-gnu': private
  /@rollup/rollup-linux-x64-musl/4.46.1:
    '@rollup/rollup-linux-x64-musl': private
  /@rollup/rollup-win32-arm64-msvc/4.46.1:
    '@rollup/rollup-win32-arm64-msvc': private
  /@rollup/rollup-win32-ia32-msvc/4.46.1:
    '@rollup/rollup-win32-ia32-msvc': private
  /@rollup/rollup-win32-x64-msvc/4.46.1:
    '@rollup/rollup-win32-x64-msvc': private
  /@types/estree/1.0.8:
    '@types/estree': private
  /@types/jsonfile/6.1.4:
    '@types/jsonfile': private
  /@types/resolve/1.20.2:
    '@types/resolve': private
  /acorn/8.15.0:
    acorn: private
  /ansi-escapes/4.3.2:
    ansi-escapes: private
  /ansi-regex/3.0.1:
    ansi-regex: private
  /ansi-styles/4.3.0:
    ansi-styles: private
  /buffer-from/1.1.2:
    buffer-from: private
  /chardet/0.7.0:
    chardet: private
  /cli-cursor/2.1.0:
    cli-cursor: private
  /cli-spinners/1.3.1:
    cli-spinners: private
  /cli-width/4.1.0:
    cli-width: private
  /color-convert/2.0.1:
    color-convert: private
  /color-name/1.1.4:
    color-name: private
  /commondir/1.0.1:
    commondir: private
  /debug/4.4.1:
    debug: private
  /deepmerge/4.3.1:
    deepmerge: private
  /emoji-regex/8.0.0:
    emoji-regex: private
  /escape-string-regexp/1.0.5:
    escape-string-regexp: private
  /estree-walker/2.0.2:
    estree-walker: private
  /external-editor/3.1.0:
    external-editor: private
  /fdir/6.4.6_picomatch@4.0.3:
    fdir: private
  /find-cache-dir/3.3.2:
    find-cache-dir: private
  /find-up/4.1.0:
    find-up: private
  /fsevents/2.3.3:
    fsevents: private
  /function-bind/1.1.2:
    function-bind: private
  /graceful-fs/4.2.11:
    graceful-fs: private
  /has-flag/4.0.0:
    has-flag: private
  /hasown/2.0.2:
    hasown: private
  /humanize-duration/3.33.0:
    humanize-duration: private
  /iconv-lite/0.4.24:
    iconv-lite: private
  /is-core-module/2.16.1:
    is-core-module: private
  /is-fullwidth-code-point/2.0.0:
    is-fullwidth-code-point: private
  /is-module/1.0.0:
    is-module: private
  /is-reference/1.2.1:
    is-reference: private
  /jsonfile/6.1.0:
    jsonfile: private
  /locate-path/5.0.0:
    locate-path: private
  /log-update/2.3.0:
    log-update: private
  /magic-string/0.30.17:
    magic-string: private
  /make-dir/3.1.0:
    make-dir: private
  /mimic-fn/1.2.0:
    mimic-fn: private
  /ms/2.1.3:
    ms: private
  /mute-stream/2.0.0:
    mute-stream: private
  /onetime/2.0.1:
    onetime: private
  /os-tmpdir/1.0.2:
    os-tmpdir: private
  /p-limit/2.3.0:
    p-limit: private
  /p-locate/4.1.0:
    p-locate: private
  /p-try/2.2.0:
    p-try: private
  /path-exists/4.0.0:
    path-exists: private
  /path-parse/1.0.7:
    path-parse: private
  /picomatch/4.0.3:
    picomatch: private
  /pkg-dir/4.2.0:
    pkg-dir: private
  /randombytes/2.1.0:
    randombytes: private
  /resolve/1.22.10:
    resolve: private
  /restore-cursor/2.0.0:
    restore-cursor: private
  /safe-buffer/5.2.1:
    safe-buffer: private
  /safer-buffer/2.1.2:
    safer-buffer: private
  /semver/7.7.2:
    semver: private
  /serialize-javascript/6.0.2:
    serialize-javascript: private
  /signal-exit/4.1.0:
    signal-exit: private
  /smob/1.5.0:
    smob: private
  /source-map-support/0.5.21:
    source-map-support: private
  /source-map/0.6.1:
    source-map: private
  /string-width/2.1.1:
    string-width: private
  /strip-ansi/4.0.0:
    strip-ansi: private
  /supports-color/7.2.0:
    supports-color: private
  /supports-preserve-symlinks-flag/1.0.0:
    supports-preserve-symlinks-flag: private
  /terser/5.43.1:
    terser: private
  /tmp/0.0.33:
    tmp: private
  /type-fest/0.21.3:
    type-fest: private
  /undici-types/7.8.0:
    undici-types: private
  /universalify/2.0.1:
    universalify: private
  /wrap-ansi/3.0.1:
    wrap-ansi: private
  /yoctocolors-cjs/2.1.2:
    yoctocolors-cjs: private
included:
  dependencies: true
  devDependencies: true
  optionalDependencies: true
injectedDeps: {}
layoutVersion: 5
nodeLinker: isolated
packageManager: pnpm@7.11.0
pendingBuilds: []
prunedAt: Mon, 28 Jul 2025 06:51:18 GMT
publicHoistPattern:
  - '*eslint*'
  - '*prettier*'
registries:
  default: https://registry.npmmirror.com/
skipped:
  - /@rollup/rollup-android-arm-eabi/4.46.1
  - /@rollup/rollup-android-arm64/4.46.1
  - /@rollup/rollup-darwin-arm64/4.46.1
  - /@rollup/rollup-freebsd-arm64/4.46.1
  - /@rollup/rollup-freebsd-x64/4.46.1
  - /@rollup/rollup-linux-arm-gnueabihf/4.46.1
  - /@rollup/rollup-linux-arm-musleabihf/4.46.1
  - /@rollup/rollup-linux-arm64-gnu/4.46.1
  - /@rollup/rollup-linux-arm64-musl/4.46.1
  - /@rollup/rollup-linux-loongarch64-gnu/4.46.1
  - /@rollup/rollup-linux-ppc64-gnu/4.46.1
  - /@rollup/rollup-linux-riscv64-gnu/4.46.1
  - /@rollup/rollup-linux-riscv64-musl/4.46.1
  - /@rollup/rollup-linux-s390x-gnu/4.46.1
  - /@rollup/rollup-linux-x64-gnu/4.46.1
  - /@rollup/rollup-linux-x64-musl/4.46.1
  - /@rollup/rollup-win32-arm64-msvc/4.46.1
  - /@rollup/rollup-win32-ia32-msvc/4.46.1
  - /@rollup/rollup-win32-x64-msvc/4.46.1
storeDir: /Users/mac/Library/pnpm/store/v3
virtualStoreDir: .pnpm
